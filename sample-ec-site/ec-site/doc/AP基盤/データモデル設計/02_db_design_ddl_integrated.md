...
(
    auth_role_id BIGSERIAL PRIMARY KEY,
...
(
    auth_account_role_id BIGSERIAL PRIMARY KEY,
...
CREATE
OR REPLACE VIEW AUTH_ACCOUNT_LOCK_LATEST_V AS
SELECT DISTINCT
ON (auth_account_id)
    auth_account_id,
    locked,
...
CREATE
OR REPLACE VIEW AUTH_ACCOUNT_EXPIRY_LATEST_V AS
SELECT DISTINCT
ON (auth_account_id)
    auth_account_id,
    event_type,
...
CREATE
OR REPLACE VIEW AUTH_ACCOUNT_LAST_LOGIN_V AS
SELECT DISTINCT
ON (auth_account_id)
    auth_account_id,
    login_at
...
CREATE
OR REPLACE VIEW AUTH_ACCOUNT_CURRENT_V AS
...
CREATE
OR REPLACE VIEW AUTH_ACCOUNT_ROLE_V AS
